...
5. Home address: street address, apartment or unit if any, city, state and ZIP code. The street address must be a house or building number plus a street name (e.g. "10 Main Street"); a city name alone is not a street address, so ask again. Apartment or unit details go in the second address line, not the street line. Store state as the 2-letter abbreviation. The ZIP must be 5 digits (or ZIP+4). If the city, state and ZIP don't seem to belong together, gently double-check with the caller.
