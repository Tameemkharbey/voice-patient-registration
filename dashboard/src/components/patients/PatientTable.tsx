...
export const PatientTable = ({ patients, onSelect, justAddedIds }: PatientTableProps): React.JSX.Element => (
      ...
      <table className="w-full min-w-[720px] text-left text-sm">
        ...
        <tbody className="divide-y divide-border">
          {patients.map((patient, index) => {
            ...
            const isNew = Boolean(justAddedIds?.has(patient.patient_id));
            return (
              <motion.tr
                key={patient.patient_id}
                initial={isNew ? { opacity: 0, y: -6 } : { opacity: 0, y: 6 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.2, delay: isNew ? 0 : Math.min(index * 0.02, 0.3) }}
                ...
                className={cn(
                  'cursor-pointer transition-colors hover:bg-accent/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset',
